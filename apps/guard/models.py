...
class BlockedIp(models.Model):
    ...
    view = models.ForeignKey('guard.ViewDetail', related_name='blocked_ips', on_delete=models.SET_NULL, null=True)
    ...
    def is_blocked(self):
        ...
        if delta.seconds > self.ban_time:
            return False
        return True

    @staticmethod
    def is_ip_blocked(ip):
        obj = BlockedIp.objects.filter(ip=ip).last()
        if obj:
...
class SecurityConfig(models.Model):
    views = models.ManyToManyField('guard.ViewDetail', related_name='config', blank=True)
